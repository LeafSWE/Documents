...
\subsection{Configurazione}
	\subsubsection{Controllo di versione}
		Il controllo di versione di documenti e file sorgente viene fatto utilizzando il software\g\ Git\g\ e la piattaforma GitHub\g. Ad ogni modifica sostanziale ad un documento o ad un file sorgente impone che a quest'ultimo venga assegnato nuovo numero di versione. 
		
		Per quanto riguarda le norme di versionamento dei file inerenti alla documentazione si rimanda alla sezione \hyperref[sec:Versionamento dei documenti]{Versionamento dei documenti} del presente documento.
		Per quanto riguarda le norme di versionamento dei file inerenti al codice si rimanda alla sezione \hyperref[sec:versionamento_file_codice]{Versionamento dei file di codice} del presente documento. 
		...
		\paragraph{Repository}
			Per organizzare e gestire meglio i file, nonché la dimensione del repository in locale, i file prodotti dal team si dividono in due categorie: documenti e codice sorgente, quindi sono mantenuti in due repository\g\ distinti.
			\begin{description}
				\item[Documents:] comprende i file necessari a redigere i documenti e gli strumenti che agiscono su di essi;
				\item[clips:] comprende i file necessari alla creazione del prodotto software.
			\end{description}
			
			\subparagraph{Struttura del repository Documents}
				
				I file all'interno del repository\g\ Documents verranno organizzati secondo questa struttura:
				\begin{itemize}
					\item /Documents
					\begin{itemize}
			...
			\subparagraph{Struttura del repository clips}
				\begin{itemize}
					\item /app
						\begin{itemize}
							\item /src
			...
			\subparagraph{Visibilità del repository}
				Per la condivisione e il versionamento dei configuration item sono stati creati due repository\g\ su GitHub\g\ all'interno del gruppo di lavoro \textit{LeafSWE} composto dai membri del gruppo \leaf, accessibile all'indirizzo \frmURI{https://github.com/LeafSWE}.
...
\newpage
		\paragraph{Procedure}
			...
			\subparagraph{Aggiornamento del repository Documents}
				Per l'aggiornamento del repository\g\ Documents è prevista la seguente procedura:
				\begin{itemize}
				...
				\begin{figure}[H]
					...
					\caption{Procedura di aggiornamento del repository Documents}\label{fig:Procedura di aggiornamento del repository} 
				\end{figure}
				
				
			\subparagraph{Aggiornamento del repository clips}
				Per l'aggiornamento del repository\g\ clips è prevista la seguente procedura:
				\begin{enumerate}
					\item Usare il comando \verb|git checkout type/nome_feature development|, sostituendo con \verb|type| la tipologia di branch:
						\begin{itemize}
							\item \verb|f| (feature) per una nuova funzionalità;
							\item \verb|d| (debug) per eseguire test e verifiche per isolare e riprodurre la procedura che causa un bug;
							\item \verb|t| (test) per sviluppare test per il prototipo;
						\end{itemize}					 
					e con \verb|nome_feature| il nome della caratteristica da aggiungere che diventerà il nome del nuovo branch;
					\item Eseguire le modifiche necessarie nel branch appena creato per l'aggiunta della nuova caratteristica usando i comandi:
						\begin{itemize}
							\item \verb|git add NomeFile|;
							\item \verb|git commit|;
							\item \verb|git push|;
						\end{itemize}
					\item Una volta che la modifica è stata completata e testata aprire la pagina github del repository ed eseguire una pull request specificando il branch \verb|development| come \textit{base} e il branch \verb|nome_feature| come \textit{compare}. Infine selezionare il pulsante \textit{'Create pull request'};
					\item Successivamente alla \textit{pull request} vengono eseguiti con lo strumento Travis CI\g\ tutti i test, se il risultato è positivo il \responsabilediprogetto è incaricato di accettare la \textit{pull request} e chiudere così il branch \verb|nome_feature| altrimenti la \textit{pull request} non è accettata e si torna al punto 2.
				\end{enumerate}
